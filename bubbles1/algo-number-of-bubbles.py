...
def detect_bubbles(image_path):
    ...
    for mode, mode_name in zip(contour_modes, contour_mode_names):
        ...
        logging.info(f"{mode_name} took {time_elapsed} seconds to identify the contours.")
        ...
        # Save the bubble sizes to a JSON file
        json_filename = f"{result_folder}/{mode_name}-detect-test2-{original_file_name}-{timestamp}.json"
        logging.info(f"Saving the bubble sizes to {json_filename}.")
        with open(json_filename, 'w') as f:
            json.dump(bubble_sizes, f)

        summary.append({
            'mode': mode_name,
            'time': time_elapsed,
            'bubbles': len(bubble_sizes)
        })
    ...
    for entry in summary:
        logging.info(f"{entry['mode']}: {entry['bubbles']} bubbles found, took {entry['time']} seconds")
